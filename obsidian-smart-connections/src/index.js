...
export default class SmartConnectionsPlugin extends Plugin {
  ...
  get smart_env_config() {
    if (!this._smart_env_config) {
      const merged_env_config = merge_env_config(
  ...
  async check_for_update() {
    try {
      ...
      if (latest_release !== this.manifest.version) {
        ...
        this.notices?.show("new_version_available", {
          version: latest_release,
        });
  ...
  async open_random_connection() {
    ...
    if (!rand_entity) {
      new Notice(
        "Cannot open random connection for non-embedded source: " +
          curr_file.path,
      );
      return;
    }
  ...
  async render_code_block(contents, container, ctx) {
    ...
    if (
      connections_settings?.results_limit &&
      (!connections_settings?.limit ||
        connections_settings?.limit !== connections_settings?.results_limit)
    ) {
      connections_settings.limit = connections_settings.results_limit;
    }
    const observed_connections_settings = create_deep_proxy(
      connections_settings,
      (updated) => {
        if (typeof ctx.replaceCode === "function")
          ctx.replaceCode(JSON.stringify(connections_settings, null, 2));
        const {
          text,
          lineStart: line_start,
          lineEnd: line_end,
        } = ctx.getSectionInfo(container) ?? {};
        contents = text
          .split("\n")
